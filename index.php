...
<style>
    ...
    * {
    ...
    .deadlines-room-item.status-ok {
        /* Зелёный цвет для ручной установки OK */
        background: rgba(39,103,73,0.14);
        border-color: rgba(39,103,73,0.45);
        color: #215c40;
    }

    .deadlines-room-item.status-auto-ok {
        /* Голубой цвет для автоматической установки OK (после опустошения) */
...
.product-btn .btn-text {
    ...
    /* Стили для отображения ежедневных проверок по этажам (как в Депатчерах) */
    #history-list.floor-mode {
        display: block;
        flex-direction: column;
    }

    /* Стили для режима "По времени" в Ежедневных (вертикальный список) */
    #history-list.time-mode {
        display: flex;
        flex-direction: column;
        gap: 8px;
    }

    #history-list.time-mode > .dispatchers-today-room-card {
        width: 100%;
        max-width: none;
    }

    #history-list.floor-mode > .dispatchers-floor-section {
        width: 100%;
        margin-bottom: 16px;
        display: block;
    }

    #history-list.floor-mode > .dispatchers-floor-section:last-child {
        margin-bottom: 0;
    }
    ...
    @media print {
        ...
        .bottom-nav, #bottomNav, #history-search-btn, #history-next-day, #history-prev-day {
            display: none !important;
        }
...
/* Голубой фон для опустошённых номеров */
.dispatchers-today-room-card.emptied {
...
<button class="btn secondary small" id="deadlines-ok-selected-btn" title="Сроки в порядке для выбранных номеров" style="display:none;"><i class="fas fa-check"></i> Сроки в порядке</button>
...
<div style="margin-top: 20px; padding: 15px; background: rgba(15,23,36,0.05); border-radius: 8px; border: 1px solid rgba(15,23,36,0.08);">
    <h4 style="margin: 0 0 10px 0; color: #333;">⚠️ Важная информация</h4>
    <ul style="margin: 0; padding-left: 20px; color: #666; font-size: 14px;">
        <li><strong>Экспорт</strong> - сохраняет все данные приложения в файл JSON</li>
        <li><strong>Импорт</strong> - загружает данные из файла JSON (текущие данные будут заменены)</li>
        <li>Рекомендуется делать резервную копию перед импортом</li>
        <li>Файлы экспорта можно использовать для переноса данных между устройствами</li>
    </ul>
</div>
</div>
</div>
...
<div style="font-weight:700; font-size:16px">История</div><div class="row" style="align-items:center; gap:6px;"><button class="btn ghost small" id="history-sort-toggle" style="white-space: nowrap; margin-right:8px;">
<i class="fas fa-sort"></i> <span id="history-sort-label">По номерам</span>
</button><div id="history-search-wrapper" style="display:flex; align-items:center; gap:6px;">
...
<!-- Кнопки переключения подразделов -->
<div style="display:flex; gap:8px; margin-bottom:12px; border-bottom:1px solid rgba(0,0,0,0.1); padding-bottom:8px;">
<button class="btn ghost small" id="history-subtab-gih" style="flex:1;">GIH</button>
<button class="btn ghost small" id="history-subtab-daily" style="flex:1;">Ежедневные</button>
</div>
<!-- Контроллер даты -->
...
<div class="card hidden" id="dispatchers-section">
    ...
    <div class="desktop-only">
        <div class="instructions" style="background:rgba(39,103,73,0.08); padding:15px; border-radius:10px; margin-bottom:15px;">
            ...
            <div class="file-path" style="font-family:monospace; background:rgba(15,23,36,0.06); padding:8px; border-radius:8px; margin-top:12px; font-size:12px; cursor:pointer;" onclick="copyFilePath()">
                <div style="margin-bottom: 8px; font-weight: 600;">Путь к файлу (кликните для копирования):</div>
...
<button class="btn ghost small" id="modal-empty">Опустошён</button>
...
<div style="text-align:center; padding:8px 0; border-bottom:1px solid rgba(0,0,0,0.1); margin-bottom:8px;">
<div style="font-weight:600; color:#666;" id="deadlines-daily-goal">Цель на сегодня: 0/0 номеров</div>
<div style="font-size:12px; color:#888; margin-top:4px;" id="deadlines-tomorrow-goal">Цель на завтра: 0 номеров</div>
</div>
...
<div class="card" style="flex:1; min-width:160px; padding:12px;">
<div class="muted" style="font-size:12px;">Пустых номеров</div>
<div style="font-size:24px; font-weight:700;color:#0a58ca" id="deadlines-count-auto-ok">0</div>
</div>
<div class="card" style="flex:1; min-width:160px; padding:12px;">
...
<div class="card" style="padding:12px;">
<div style="font-weight:600; margin-bottom:8px;">📊 Динамика проверки минибаров</div>
<div class="muted" style="font-size:12px; margin-bottom:12px;">Количество номеров, переведенных в каждый статус по дням (последние 30 дней)</div>
<canvas id="deadlines-activity-chart" width="100%" height="250" style="width:100%; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); display:block;"></canvas>
</div>
</div>
</div>
</div>
<script>
    ...
    function copyFilePath() {
      const filePath = 'G:\\Food and Beverage\\F&B Service\\Outlets\\Room Service\\04 MB\\OperaPrint.pdf';
      copyText(filePath);
      showToast('Путь к файлу скопирован в буфер обмена', 'success');
    }
    </script>
<script>
                ...
                const appData = {
                    ...
                    gihRecords: [],
                    gihHistory: [],
                    deadlinesDailyStats: {}, // { 'YYYY-MM-DD': { ok: count, products: count } }
                    products: {
                ...
                // Делаем appData доступным глобально для функции печати
    ...
    try {
        // НЕ сохраняем в localStorage - данные всегда должны быть в БД
        ...
        // Сначала читаем текущие данные из Firebase, чтобы не потерять другие данные
        const currentSnap = await database.ref('minibarData').once('value');
        const currentData = currentSnap.val() || {};
        
        // Мержим данные: сохраняем текущие данные из Firebase и обновляем только измененные
        const dataToSave = {
            ...currentData,  // Сохраняем все существующие данные из Firebase
            ...appData,      // Обновляем локальные данные
            gihHistory: mergedHistory,  // Используем объединенную историю
        };
        
        // Убеждаемся, что deadlinesDailyStats включен в данные для сохранения
        // Используем локальные данные, если они есть, иначе данные из Firebase
        if (appData.deadlinesDailyStats) {
            dataToSave.deadlinesDailyStats = appData.deadlinesDailyStats;
            console.log('✅ deadlinesDailyStats включен в данные для сохранения:', appData.deadlinesDailyStats);
        } else if (currentData.deadlinesDailyStats) {
            // Если локальной статистики нет, сохраняем ту, что есть в Firebase
            dataToSave.deadlinesDailyStats = currentData.deadlinesDailyStats;
            console.log('✅ Используем deadlinesDailyStats из Firebase:', currentData.deadlinesDailyStats);
        } else {
            console.warn('⚠️ deadlinesDailyStats отсутствует в appData и Firebase при сохранении');
        }
        
        // 4) сохраняем целиком, но уже с объединённой историей и сохраненными данными
        await database.ref('minibarData').set(dataToSave);
        console.log('✅ Данные (с объединённой историей) сохранены в Firebase');
        console.log('📊 Сохранено deadlinesDailyStats:', JSON.stringify(dataToSave.deadlinesDailyStats, null, 2));
        
        // Проверяем, что данные действительно сохранились
        const verifySnap = await database.ref('minibarData/deadlinesDailyStats').once('value');
        const verified = verifySnap.val();
        console.log('✅ Проверка сохранения: deadlinesDailyStats в Firebase:', JSON.stringify(verified, null, 2));
    } catch (error) {
        console.error('Ошибка сохранения в Firebase:', error);
        // Данные не сохраняются локально - только в БД
    }
}
    ...
    database.ref('minibarData').on('value', (snapshot) => {
        ...
        if (data && !isUpdatingFromFirebase) {
            ...
            // НЕ сохраняем локальные изменения при обновлении из Firebase
            // Данные всегда должны браться из БД
            Object.assign(appData, data);

            // Синхронизируем deadlinesDailyStats
            deadlinesDailyStats = appData.deadlinesDailyStats || {};

            // Обновляем статистику если модальное окно открыто
            // НЕ вызываем ensureDailyGoalCalculated() при обновлении из Firebase,
            // чтобы избежать циклических обновлений
            if (deadlinesStatsModalOpen) {
                // Используем существующую функцию renderDeadlinesStats, но без пересчета цели
                // Просто обновляем отображение с текущими данными
                const data = computeDeadlinesSummary();
                const okEl = $('#deadlines-count-ok');
                const autoOkEl = $('#deadlines-count-auto-ok');
                const prodEl = $('#deadlines-count-products');
                const neutralEl = $('#deadlines-count-neutral');
                if (okEl) okEl.textContent = data.ok;
                if (autoOkEl) autoOkEl.textContent = data.auto_ok;
                if (prodEl) prodEl.textContent = data.products;
                if (neutralEl) neutralEl.textContent = data.neutral;
                
                // Обновляем отображение цели без пересчета (используем данные из Firebase)
                updateDeadlinesDailyGoal(data);
                
                // Обновляем график активности
                renderDeadlinesActivityChart();
            }
            ...
            updateUIFromData();
                ...
                // Функция для загрузки статистики из Firebase (одноразово)
                // Теперь загружается из minibarData через loadFromFirebase
                function loadDeadlinesStatsFromFirebase() {
                    // Статистика уже загружена через loadFromFirebase из minibarData
                    deadlinesDailyStats = appData.deadlinesDailyStats || {};
                    console.log('Загружена статистика сроков из minibarData:', deadlinesDailyStats);
                    return Promise.resolve(deadlinesDailyStats);
                }

                // Настраиваем realtime синхронизацию для deadlinesDailyStats
                // Теперь синхронизация происходит через setupFirebaseListener для minibarData
                let lastLocalChangeTime = 0; // Время последнего локального изменения

                function setupDeadlinesStatsRealtimeListener() {
                    // Статистика синхронизируется через setupFirebaseListener для minibarData
                    // Обновляем deadlinesDailyStats при изменении appData
                    const checkStatsUpdate = () => {
                        if (appData.deadlinesDailyStats) {
                            const currentTime = Date.now();
                            
                            // Если прошло менее 2 секунд с момента локального изменения,
                            // игнорируем обновление из Firebase
                            if (currentTime - lastLocalChangeTime < 2000) {
                                return;
                            }

                            if (JSON.stringify(appData.deadlinesDailyStats) !== JSON.stringify(deadlinesDailyStats)) {
                                deadlinesDailyStats = appData.deadlinesDailyStats || {};

                                // Если данные изменились, обновляем UI если модальное окно открыто
                                if (deadlinesStatsModalOpen) {
                                    console.log('Обновлена статистика сроков из minibarData:', deadlinesDailyStats);
                    ...
                    // Проверяем при каждом обновлении appData (через setupFirebaseListener)
                    // Это будет вызываться автоматически при изменении minibarData
                }
...
accisesToRender.forEach(renderAcciseItem);
    ...
    renderDispatchersRooms();
                ...
                function persistLocalSnapshot() {
                    // НЕ сохраняем данные в localStorage - данные всегда должны быть в БД
                    // try {
                    //     localStorage.setItem('hotelMinibarData', JSON.stringify(appData));
                    // } catch (e) {
  ...
  if (navigator.clipboard && navigator.clipboard.writeText) {
    ...
    if (callback) callback();
                ...
                function renderAcciseItem(a) {
                    ...
                    el.innerHTML = `
                                            ...
                                            <div class="accise-footer" style="position:absolute; left:12px; bottom:8px;">
                                                <small class="muted">${new Date(a.ts).toLocaleString('ru-RU', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</small>
                                            </div>
                                        `;
                ...
                const DEADLINE_STATUS = {
                    NEUTRAL: 'neutral',
                    PRODUCTS: 'products',
                    OK: 'ok',           // Ручная установка OK (зелёный)
                    AUTO_OK: 'auto_ok'  // Автоматическая установка после опустошения (голубой)
                };
    ...
    // Статистика переходов номеров по дням (загружается из Firebase)
    let deadlinesDailyStats = {}; // { 'YYYY-MM-DD': { ok: count, products: count } }
    ...
    // Сбрасываем все номера, кроме пустых (AUTO_OK)
    (appData.rooms || []).forEach(room => {
        if (room && room.deadlinesStatus !== DEADLINE_STATUS.AUTO_OK) {
            room.products = {};
    ...
    // Очищаем статистику переходов локально и в Firebase
    deadlinesDailyStats = {};
    appData.deadlinesDailyStats = {};

    // Сохраняем через общую функцию сохранения (в minibarData)
    saveToFirebase();
...
function toggleFloorSelection(floorNumber) {
    ...
    renderRooms();
                ...
                function initSettings() {
                    ...
                    const importBtn = document.getElementById('import-database-btn');
                    if (importBtn) {
                ...
                function exportDatabase() {
                    try {
                        // Создаем копию данных без локальных акциз
                        const { accises, ...exportData } = appData;
                        ...
                        link.download = `minibars-backup-${new Date().toISOString().split('T')[0]}.json`;
                ...
                function handleImportFile(event) {
                    ...
                    reader.onload = (e) => {
                        try {
                            ...
                            // Сохраняем локальные акцизы
                            const localAccises = appData.accises || [];

                            // Обновляем данные
                            Object.assign(appData, importedData);

                            // Восстанавливаем локальные акцизы
                            appData.accises = localAccises;
                            ...
                            saveToFirebase().then(() => {
                                ...
                                console.error('Ошибка сохранения после импорта:', error);
                                showToast('Ошибка сохранения импортированных данных', 'error');
                            });

                        } catch (error) {
                            console.error('Ошибка импорта:', error);
                            showToast('Ошибка при импорте базы данных: ' + error.message, 'error');
                        }
                    };
                ...
                function initSroki() {
                    ...
                    const emptyBtn = $('#modal-empty');
                    if (emptyBtn) emptyBtn.addEventListener('click', markDeadlinesEmpty);
                    ...
                    const okSelectedBtn = $('#deadlines-ok-selected-btn');
                    if (okSelectedBtn) okSelectedBtn.addEventListener('click', markSelectedDeadlinesOk);
    ...
    if (resetBtnMobile) resetBtnMobile.addEventListener('click', resetAllDeadlines);
                ...
                function markDeadlinesEmpty() {
                    if (currentDeadlineRoomNumber == null) return;
                    const room = ensureRoomRecord(currentDeadlineRoomNumber);
                    commitDeadlineChanges(room, {}, DEADLINE_STATUS.AUTO_OK);
                    closeModal();
                }
                function markSelectedDeadlinesOk() {
                    console.log('=== markSelectedDeadlinesOk called ===');
                    if (deadlinesSelectedRooms.size === 0) return;
                    const selectedRooms = Array.from(deadlinesSelectedRooms);
                    console.log('Processing rooms:', selectedRooms);
                    selectedRooms.forEach(roomStr => {
                        const roomNumber = Number(roomStr);
                        const room = ensureRoomRecord(roomNumber);
                        console.log(`Processing room ${roomNumber}, current status: ${room.deadlinesStatus}`);
                        
                        // Сохраняем предыдущий статус для проверки изменения
                        const previousStatus = room.deadlinesStatus;
                        
                        room.products = {};
                        room.deadlinesStatus = DEADLINE_STATUS.OK;
                        console.log(`Set status to OK: ${room.deadlinesStatus}`);
                        normalizeRoomDeadlineStatus(room);
                        console.log(`After normalize: ${room.deadlinesStatus}`);

                        // Записываем статистику только если статус действительно изменился
                        recordDeadlineStatusChange(room.deadlinesStatus, previousStatus).catch(error => {
                    ...
                    updateDeadlinesSelectedCount();
                    updateDeadlinesStatsIfOpen();
                    console.log('=== markSelectedDeadlinesOk finished ===');
                }
                ...
                function commitDeadlineChanges(room, products, status) {
                    console.log(`commitDeadlineChanges: room ${room.number}, requested status: ${status}`);
                    
                    // Сохраняем предыдущий статус для синхронизации с разделом "Пустые"
                    const previousStatus = room.deadlinesStatus;
                    
                    room.products = JSON.parse(JSON.stringify(products || {}));
                    room.deadlinesStatus = status;
                    console.log(`Before normalize: room ${room.number} status = ${room.deadlinesStatus}`);
                    normalizeRoomDeadlineStatus(room);
                    console.log(`After normalize: room ${room.number} final status = ${room.deadlinesStatus}`);

                    // Синхронизация с разделом "Пустые" (vkEmptiedRooms)
                    const finalStatus = room.deadlinesStatus;
                    const roomNumber = String(room.number || room.name);
                    
                    if (finalStatus === DEADLINE_STATUS.AUTO_OK && previousStatus !== DEADLINE_STATUS.AUTO_OK) {
                        // Номер помечен как пустой - добавляем в vkEmptiedRooms
                        const timestamp = Math.floor(Date.now() / 1000);
                        if (database && typeof database.ref === 'function') {
                            database.ref(`vkEmptiedRooms/${roomNumber}`).set({ ts: timestamp })
                                .then(() => {
                                    console.log(`Добавлен номер ${roomNumber} в раздел "Пустые"`);
                                })
                                .catch(error => {
                                    console.error('Ошибка добавления номера в раздел "Пустые":', error);
                                });
                        }
                    } else if (previousStatus === DEADLINE_STATUS.AUTO_OK && finalStatus !== DEADLINE_STATUS.AUTO_OK) {
                        // Номер больше не пустой - удаляем из vkEmptiedRooms
                        if (database && typeof database.ref === 'function') {
                            database.ref(`vkEmptiedRooms/${roomNumber}`).remove()
                                .then(() => {
                                    console.log(`Удален номер ${roomNumber} из раздела "Пустые"`);
                                })
                                .catch(error => {
                                    console.error('Ошибка удаления номера из раздела "Пустые":', error);
                                });
                        }
                    }

                    // Записываем статистику только если статус действительно изменился
                    recordDeadlineStatusChange(finalStatus, previousStatus).catch(error => {
                        console.error('❌ Ошибка записи статистики:', error);
                    });
                ...
                // Функция для записи изменения статуса в статистику
                // Функция для записи изменения статуса в статистику
                async function recordDeadlineStatusChange(finalStatus, previousStatus) {
                    console.log('=== recordDeadlineStatusChange called ===');
                    console.log('finalStatus:', finalStatus);
                    console.log('previousStatus:', previousStatus);
                    console.log('DEADLINE_STATUS.OK:', DEADLINE_STATUS.OK);
                    console.log('DEADLINE_STATUS.PRODUCTS:', DEADLINE_STATUS.PRODUCTS);
                    console.log('DEADLINE_STATUS.AUTO_OK:', DEADLINE_STATUS.AUTO_OK);

                    // Считаем только финальные статусы (OK, AUTO_OK, PRODUCTS)
                    const validStatuses = [DEADLINE_STATUS.OK, DEADLINE_STATUS.AUTO_OK, DEADLINE_STATUS.PRODUCTS];
                    console.log('validStatuses:', validStatuses);
                    console.log('finalStatus in validStatuses:', validStatuses.includes(finalStatus));

                    if (!validStatuses.includes(finalStatus)) {
                        console.log('❌ Ignoring intermediate status');
                        return; // Игнорируем промежуточные статусы
                    }

                    // Проверяем, изменился ли статус
                    // Если статус не изменился, не записываем статистику
                    // Если previousStatus не указан (undefined/null), считаем что это первое изменение и записываем
                    if (previousStatus !== undefined && previousStatus !== null) {
                        if (previousStatus === finalStatus) {
                            console.log('❌ Статус не изменился, пропускаем запись статистики');
                            return;
                        }

                        // Также проверяем переходы между эквивалентными статусами (OK и AUTO_OK)
                        // Если был OK и стал AUTO_OK (или наоборот), это тоже не считается изменением для статистики
                        const okStatuses = [DEADLINE_STATUS.OK, DEADLINE_STATUS.AUTO_OK];
                        const wasOk = okStatuses.includes(previousStatus);
                        const isOk = okStatuses.includes(finalStatus);
                        if (wasOk && isOk) {
                            console.log('❌ Переход между OK и AUTO_OK, пропускаем запись статистики');
                            return;
                        }
                    }
                    ...
                    // Всегда берем данные из БД (appData.deadlinesDailyStats синхронизируется с Firebase)
                    // Сначала читаем текущие данные из Firebase, чтобы не потерять данные
                    const currentStatsSnap = await database.ref('minibarData/deadlinesDailyStats').once('value');
                    const currentStatsFromDB = currentStatsSnap.val() || {};
                    
                    // Мержим данные: сначала данные из БД, затем обновляем только сегодняшнюю дату
                    const updatedStats = { ...currentStatsFromDB };
                    console.log('Current stats from DB:', updatedStats[dateKey]);

                    if (!updatedStats[dateKey]) {
                        updatedStats[dateKey] = { ok: 0, products: 0, dailyGoal: 0 };
                    }

                    // Увеличиваем соответствующий счетчик
                    if (finalStatus === DEADLINE_STATUS.OK || finalStatus === DEADLINE_STATUS.AUTO_OK) {
                        updatedStats[dateKey].ok++;
                        console.log(`✅ Статистика: OK +1 (итого: ${updatedStats[dateKey].ok})`);
                    } else if (finalStatus === DEADLINE_STATUS.PRODUCTS) {
                        updatedStats[dateKey].products++;
                        console.log(`✅ Статистика: PRODUCTS +1 (итого: ${updatedStats[dateKey].products})`);
                    }

                    console.log('Updated stats:', updatedStats[dateKey]);

                    // Очищаем старые записи (оставляем только последние 30 дней)
                    cleanupOldDeadlineStats(updatedStats);

                    // Обновляем локальную переменную и appData (синхронизируем с БД)
                    deadlinesDailyStats = updatedStats;
                    appData.deadlinesDailyStats = updatedStats;

                    // Устанавливаем время последнего локального изменения
                    lastLocalChangeTime = Date.now();
                    
                    // Сохраняем статистику с мержанными данными
                    console.log('📊 Сохраняем обновленную статистику в Firebase:', updatedStats);
                    await database.ref('minibarData/deadlinesDailyStats').set(updatedStats);

                    // Обновляем график если модальное окно открыто
                    if (deadlinesStatsModalOpen) {
                        renderDeadlinesActivityChart();
                    }

                    console.log('=== recordDeadlineStatusChange finished ===');
                }

                // Функция для очистки старых записей статистики
                function cleanupOldDeadlineStats(stats) {
...
section.appendChild(title);
                        ...
                        roomsOnFloor.forEach(num => {
                            ...
                            roomEl.classList.toggle('status-auto-ok', status === DEADLINE_STATUS.AUTO_OK);
                            ...
                            // Номера со статусом AUTO_OK теперь остаются активными
                            ...
                            } else if (status === DEADLINE_STATUS.OK || status === DEADLINE_STATUS.AUTO_OK) {
                ...
                function normalizeRoomDeadlineStatus(room) {
                    ...
                    if (room.deadlinesStatus === DEADLINE_STATUS.OK || room.deadlinesStatus === DEADLINE_STATUS.AUTO_OK) {
                        return room;
                    }
                ...
                function toggleDeadlinesSelectMode() {
                    ...
                    if (okSelectedBtn) okSelectedBtn.style.display = deadlinesSelectMode && deadlinesSelectedRooms.size > 0 ? 'inline-flex' : 'none';
                }
                ...
                function toggleDeadlineRoomSelection(roomNumber) {
                    ...
                    // В мобильном режиме выключаем режим выбора после клика на номер
                    if (window.innerWidth <= 920 && deadlinesSelectMode) {
                        toggleDeadlinesSelectMode();
                    }
                }
                ...
                function updateDeadlinesSelectedCount() {
                    ...
                    if (printBtn) printBtn.style.display = deadlinesSelectMode && count > 0 ? 'inline-flex' : 'none';
                    if (okSelectedBtn) okSelectedBtn.style.display = deadlinesSelectMode && count > 0 ? 'inline-flex' : 'none';
                }
...
const filteredRooms = selectedRooms.filter(num => {
    ...
    return room.deadlinesStatus !== DEADLINE_STATUS.OK && room.deadlinesStatus !== DEADLINE_STATUS.AUTO_OK;
});
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                function computeDeadlinesSummary() {
                    const summary = {
                        ok: 0,
                        auto_ok: 0,  // Пустые номера (опустошённые)
                        products: 0,
                        neutral: 0,
                        productTotals: {}
                    };
                    (appData.rooms || []).forEach(room => {
                        ...
                        else if (status === DEADLINE_STATUS.AUTO_OK) summary.auto_ok += 1;
                ...
                // Функция для выбора номеров с выбранным продуктом
                function selectRoomsWithProduct(productKey) {
                    const roomsWithProduct = (appData.rooms || []).filter(room => {
                        if (!room || !room.products) return false;
                        return room.products[productKey] && room.products[productKey] > 0;
                    });

                    if (roomsWithProduct.length === 0) {
                        alert(`Нет номеров с выбранным продуктом`);
                        return;
                    }

                    // Включаем режим выбора, если он не включен
                    if (!deadlinesSelectMode) {
                        toggleDeadlinesSelectMode();
                    }
                    ...
                    // Выбираем все номера с этим продуктом
                    roomsWithProduct.forEach(room => {
                    ...
                    // Закрываем модальное окно статистики ПЕРВЫМ ДЕЛОМ
                    closeDeadlinesStatsModal();

                    // Обновляем счетчик выбранных номеров
                    updateDeadlinesSelectedCount();

                    // Перерисовываем номера с выделением выбранных
                    renderRooms();

                    // Показываем уведомление
                    const productName = appData.products[productKey] || productKey;
                    showToast(`Выбрано ${roomsWithProduct.length} номеров с продуктом "${productName}"`, 'success');
                }
                ...
                function renderDeadlinesStats() {
                    ...
                    // Проверяем и рассчитываем цель на сегодня (если нужно)
                    // Цель пересчитывается только если её нет или если изменилось количество номеров
                    ensureDailyGoalCalculated().catch(error => {
                        console.error('❌ Ошибка проверки цели:', error);
                    });

                    // Отображаем цель на сегодня
                    updateDeadlinesDailyGoal(data);
                    ...
                    entries.forEach(([key, count]) => {
                        ...
                        // Добавляем обработчик клика для выбора номеров с этим продуктом
                        chip.addEventListener('click', () => {
                            selectRoomsWithProduct(key);
                        });

                        // Добавляем hover эффекты
                        chip.addEventListener('mouseenter', () => {
                            chip.style.background = 'rgba(15,23,36,0.12)';
                            chip.style.borderColor = 'rgba(15,23,36,0.2)';
                        });
                        chip.addEventListener('mouseleave', () => {
                            chip.style.background = 'rgba(15,23,36,0.06)';
                            chip.style.borderColor = 'rgba(15,23,36,0.08)';
                        });
                ...
                // Функция для расчета ежедневной цели (вызывается один раз в день)
                async function calculateDailyGoal() {
                    const today = new Date();
                    ...
                    // Получаем текущие данные о номерах
                    const data = computeDeadlinesSummary();

                    // Количество номеров, которые нужно перевести в статус ok
                    // Это все номера, которые НЕ в статусе ok (нейтральные + продукты, пустые не учитываем)
                    const remainingToOk = data.neutral + data.products;
                    
                    console.log('📊 Расчет цели на сегодня:', {
                        dateKey,
                        currentDay,
                        ...
                        ok: data.ok,
                        auto_ok: data.auto_ok
                    });

                    let target = 0;
                    let targetDate = 20; // По умолчанию цель до 20 числа

                    // Проверяем, достигнута ли цель к 20 числу
                    if (currentDay < 20) {
                        // До 20 числа: цель = оставшиеся номера / дни до 20 (включительно)
                        const daysUntil20th = 20 - currentDay + 1;
                        target = remainingToOk > 0 ? Math.ceil(remainingToOk / daysUntil20th) : 0;
                        targetDate = 20;
                        console.log(`📅 До 20 числа: ${remainingToOk} номеров / ${daysUntil20th} дней = ${target}`);
                    } else {
                        // После 20 числа: пересчитываем до последнего дня месяца
                        const lastDayOfMonth = new Date(currentYear, currentMonth + 1, 0).getDate();
                        const daysUntilEndOfMonth = lastDayOfMonth - currentDay + 1;
                        
                        // Если цель не достигнута к 20 числу, пересчитываем на оставшиеся дни месяца
                        if (remainingToOk > 0 && daysUntilEndOfMonth > 0) {
                            target = Math.ceil(remainingToOk / daysUntilEndOfMonth);
                        } else {
                            target = 0;
                        }
                        targetDate = lastDayOfMonth;
                        console.log(`📅 После 20 числа: ${remainingToOk} номеров / ${daysUntilEndOfMonth} дней = ${target}`);
                    }
                    ...
                    if (!updatedStats[dateKey]) {
                        ...
                        updatedStats[dateKey] = { ok: 0, products: 0, dailyGoal: target };
                    } else {
                        // Сохраняем существующие счетчики ok и products, обновляем только цель
                        updatedStats[dateKey] = { 
                            ...updatedStats[dateKey],
                            ok: updatedStats[dateKey].ok || 0,
                            products: updatedStats[dateKey].products || 0,
                            dailyGoal: target
                        };
                    }

                    // Обновляем локальные данные (синхронизируем с БД)
                    deadlinesDailyStats = updatedStats;
                    appData.deadlinesDailyStats = updatedStats;
                    ...
                    // Сохраняем статистику с мержанными данными
                    console.log('🎯 Сохраняем обновленную цель в Firebase:', updatedStats);
                    await database.ref('minibarData/deadlinesDailyStats').set(updatedStats);

                    console.log(`Рассчитана цель на ${dateKey}: ${target} номеров (цель до ${targetDate} числа)`);
                    return target;
                }

                // Функция для проверки и расчета цели на сегодня (если её нет)
                async function ensureDailyGoalCalculated() {
                    const today = new Date();
                    const dateKey = today.getFullYear() + '-' +
                                   String(today.getMonth() + 1).padStart(2, '0') + '-' +
                                   String(today.getDate()).padStart(2, '0');

                    // Всегда читаем данные из БД перед проверкой
                    try {
                        const statsSnap = await database.ref('minibarData/deadlinesDailyStats').once('value');
                        const statsFromDB = statsSnap.val() || {};
                        
                        // Синхронизируем данные из БД
                        deadlinesDailyStats = statsFromDB;
                        appData.deadlinesDailyStats = statsFromDB;
                        
                        const todayStats = statsFromDB[dateKey];
                        
                        console.log('🔍 Проверка цели на сегодня:', {
                            dateKey,
                            todayStats,
                            hasDailyGoal: todayStats && typeof todayStats.dailyGoal !== 'undefined',
                            dailyGoal: todayStats?.dailyGoal
                        });

                        // Если цели на сегодня нет или она не определена, рассчитываем
                        // НЕ пересчитываем, если цель уже установлена (даже если она равна 0)
                        const needsRecalculation = !todayStats || typeof todayStats.dailyGoal === 'undefined';
                        
                        if (needsRecalculation) {
                            const currentData = computeDeadlinesSummary();
                            const currentRemainingToOk = currentData.neutral + currentData.products;
                            console.log('❌ Цель на сегодня не рассчитана, рассчитываем...', {
                                todayStats,
                                currentRemainingToOk,
                                neutral: currentData.neutral,
                                products: currentData.products
                            });
                            await calculateDailyGoal();
                        } else {
                            console.log('✅ Цель на сегодня уже рассчитана:', todayStats.dailyGoal, 'не пересчитываем');
                        }
                    } catch (error) {
                        console.error('❌ Ошибка проверки цели:', error);
                    }
                ...
                // Функция для отображения целей на сегодня и завтра
                function updateDeadlinesDailyGoal(data) {
                    const goalEl = $('#deadlines-daily-goal');
                    const tomorrowGoalEl = $('#deadlines-tomorrow-goal');
                    if (!goalEl || !tomorrowGoalEl) return;
                    ...
                    // Количество номеров, переведенных в OK сегодня
                    const todayStats = deadlinesDailyStats[dateKey] || { ok: 0, products: 0, dailyGoal: 0 };
                    const completed = todayStats.ok;
                    const target = todayStats.dailyGoal || 0;

                    // Отображаем цель на сегодня
                    goalEl.textContent = `Цель на сегодня: ${completed}/${target} номеров`;

                    // Цветовое оформление для цели на сегодня
                    if (target === 0) {
                        goalEl.style.color = '#1b7a2b'; // Зеленый - цель достигнута
                    } else if (completed >= target) {
                        goalEl.style.color = '#1b7a2b'; // Зеленый - цель достигнута
                    } else if (completed >= target * 0.8) {
                        goalEl.style.color = '#ffc107'; // Желтый - близко к цели
                    } else {
                        goalEl.style.color = '#666'; // Серый - цель не достигнута
                    }

                    // Рассчитываем цель на завтра (динамически с учетом сегодняшнего прогресса)
                    let tomorrowTarget = 0;
                    
                    // Количество номеров, которые нужно перевести в статус ok (нейтральные + продукты, пустые не учитываем)
                    // data уже отражает текущее состояние, включая сегодняшние переводы
                    const remainingToOk = data.neutral + data.products;
                    
                    // Вычитаем количество номеров, переведенных сегодня в OK
                    // чтобы рассчитать, сколько останется на завтра
                    const remainingAfterToday = Math.max(0, remainingToOk - completed);
                    
                    console.log('🔮 Расчет цели на завтра:', {
                        remainingToOk,
                        completed,
                        remainingAfterToday,
                        currentDay
                    ...
                    if (currentDay < 20) {
                        // До 20 числа: оставшиеся дни (от завтра до 20 числа включительно)
                        const remainingDays = 20 - currentDay;
                        
                        if (remainingDays > 0 && remainingAfterToday > 0) {
                            // Цель на завтра = оставшиеся номера / оставшиеся дни
                            tomorrowTarget = Math.ceil(remainingAfterToday / remainingDays);
                            console.log(`🔮 До 20 числа: ${remainingAfterToday} номеров / ${remainingDays} дней = ${tomorrowTarget}`);
                        }
                    } else {
                        // После 20 числа: пересчитываем до последнего дня месяца
                        const lastDayOfMonth = new Date(currentYear, currentMonth + 1, 0).getDate();
                        ...
                        if (daysUntilEndOfMonth > 0 && remainingAfterToday > 0) {
                            // Цель на завтра = оставшиеся номера / оставшиеся дни до конца месяца
                            tomorrowTarget = Math.ceil(remainingAfterToday / daysUntilEndOfMonth);
                            console.log(`🔮 После 20 числа: ${remainingAfterToday} номеров / ${daysUntilEndOfMonth} дней = ${tomorrowTarget}`);
                        }
                    }

                    // Отображаем цель на завтра
                    tomorrowGoalEl.textContent = `Цель на завтра: ${tomorrowTarget} номеров`;
                }
                ...
                function openDeadlinesStatsModal() {
                    ...
                    // Рисуем график после открытия модального окна
                    setTimeout(() => renderDeadlinesActivityChart(), 100);
                }
                ...
                function updateDeadlinesStatsIfOpen() {
                    ...
                    // Синхронизируем данные перед обновлением
                    if (appData.deadlinesDailyStats) {
                        deadlinesDailyStats = appData.deadlinesDailyStats;
                    }
                    renderDeadlinesStats();
                ...
                // Функция для отрисовки графика активности
                function renderDeadlinesActivityChart() {
                    const canvas = document.getElementById('deadlines-activity-chart');
                    if (!canvas) return;

                    const ctx = canvas.getContext('2d');
                    if (!ctx) return;

                    // Устанавливаем размеры canvas (на всю ширину контейнера)
                    ...
                    // Создаем массив для 30 дней (от 29 дней назад до сегодня)
                    const today = new Date();
                    const dates = [];
                    const okData = [];
                    const productsData = [];
                    ...
                    for (let i = 29; i >= 0; i--) {
                        ...
                        dates.push(dateKey);
                        // Используем актуальные данные из appData, синхронизируем с локальной переменной
                        const currentStats = appData.deadlinesDailyStats || deadlinesDailyStats || {};
                        deadlinesDailyStats = currentStats;
                        const dayData = currentStats[dateKey] || { ok: 0, products: 0 };
                        okData.push(dayData.ok || 0);
                        productsData.push(dayData.products || 0);
                    }

                    // Находим максимальное значение для шкалы Y
                    const allValues = [...okData, ...productsData];
                    const maxValue = Math.max(...allValues, 1);
                    const hasData = allValues.some(v => v > 0);

                    // Если совсем нет данных, показываем сообщение
                    if (!hasData) {
                        // Рисуем фон в стиле сайта
                        const gradient = ctx.createLinearGradient(0, 0, 0, canvasHeight);
                        gradient.addColorStop(0, 'rgba(255,255,255,0.98)');
                        gradient.addColorStop(1, 'rgba(250,250,250,0.98)');
                        ctx.fillStyle = gradient;
                        ctx.fillRect(0, 0, canvasWidth, canvasHeight);

                        // Рисуем скругленную рамку
                        ctx.strokeStyle = 'rgba(15,23,36,0.06)';
                        ctx.lineWidth = 1;
                        const radius = 8;
                        ctx.beginPath();
                        ctx.moveTo(radius, 0);
                        ...
                        // Сообщение
                        ctx.fillStyle = 'rgba(15,23,36,0.6)';
                        ctx.font = '14px Inter, -apple-system, BlinkMacSystemFont, sans-serif';
                        ctx.textAlign = 'center';
                        ctx.fillText('📊 Статистика появится после первой проверки', canvasWidth / 2, canvasHeight / 2 - 8);

                        ctx.font = '12px Inter, -apple-system, BlinkMacSystemFont, sans-serif';
                        ctx.fillStyle = 'rgba(15,23,36,0.4)';
                        ctx.fillText('Начните проверять номера, и здесь отобразится динамика', canvasWidth / 2, canvasHeight / 2 + 16);

                        return;
                    }

                    // Размеры графика с улучшенными отступами
                    ...
                    bgGradient.addColorStop(1, 'rgba(250,250,250,0.98)');
                    ctx.fillStyle = bgGradient;
                    ctx.fillRect(0, 0, canvasWidth, canvasHeight);

                    // Рисуем оси в стиле сайта
                    ctx.strokeStyle = 'rgba(15,23,36,0.08)';
                    ctx.lineWidth = 1.5;
                    ctx.beginPath();
                    ...
                    // Рисуем горизонтальную сетку (более тонкая и аккуратная)
                    ctx.strokeStyle = 'rgba(15,23,36,0.04)';
                    ctx.lineWidth = 1;
                    ctx.setLineDash([3, 3]);
                    ...
                    // Подписи по оси Y (улучшенная типографика)
                    ...
                    for (let i = 0; i <= gridLines; i++) {
                        ...
                        ctx.fillText(value.toString(), margin.left - 12, y + 4);
                    }

                    // Цвета в стиле сайта
                    const okColor = '#1b7a2b'; // Зеленый из стиля сайта
                    const productsColor = '#c12a35'; // Красный из стиля сайта

                    // Функция для рисования линии с заливкой под ней
                    const drawLineWithArea = (data, color, progress = 1) => {
                        const points = [];
                        ...
                        ctx.lineTo(lastPoint.x, margin.top + chartHeight);
                        ctx.closePath();
                        ctx.fill();
                        ...
                        // Рисуем точки только для значений > 0
                        points.forEach(point => {
                            if (point.value > 0) {
                                ...
                                ctx.textAlign = 'center';
                                const textY = Math.max(point.y - 18, margin.top + 12);
                    ...
                    const animate = () => {
                        ...
                        drawLineWithArea(productsData, productsColor, animationProgress);
                        drawLineWithArea(okData, okColor, animationProgress);

                        if (animationProgress < 1) {
                            requestAnimationFrame(animate);
                        } else {
                            // После анимации рисуем подписи и легенду
                            drawLabelsAndLegend();
                        }
                    };

                    const drawLabelsAndLegend = () => {
                        // Рисуем подписи дат по оси X (каждые 5 дней)
                        ctx.fillStyle = 'rgba(15,23,36,0.5)';
                        ctx.font = '11px Inter, -apple-system, BlinkMacSystemFont, sans-serif';
                        ctx.textAlign = 'center';
                        ...
                        for (let i = 0; i < dates.length; i += labelStep) {
                            ...
                            ctx.fillText(dateStr, x, canvasHeight - margin.bottom + 20);
                        }

                        // Рисуем легенду в стиле сайта
                        const legendX = margin.left + 10;
                        const legendY = margin.top - 5;
                        const legendItemHeight = 20;
                        const legendSpacing = 12;

                        // Фон легенды с закругленными углами
                        ...
                        // Элемент легенды для OK
                        const okLegendY = legendY + 12;
                        ctx.fillStyle = okColor;
                        ctx.fillRect(legendX + 12, okLegendY - 6, 12, 12);
                        ctx.fillStyle = 'rgba(15,23,36,0.7)';
                        ctx.font = '12px Inter, -apple-system, BlinkMacSystemFont, sans-serif';
                        ctx.textAlign = 'left';
                        ctx.fillText('Сроки в порядке', legendX + 30, okLegendY + 4);

                        // Элемент легенды для PRODUCTS
                        const productsLegendY = legendY + 28;
                        ctx.fillStyle = productsColor;
                        ctx.fillRect(legendX + 12, productsLegendY - 6, 12, 12);
                        ctx.fillStyle = 'rgba(15,23,36,0.7)';
                        ctx.fillText('Нужно заменить', legendX + 30, productsLegendY + 4);
                    };
...
saveBtn.addEventListener('click', () => {
    ...
    // Обновляем состояние кнопки печати после сохранения
    if (typeof updateGIHPrintButtonState === 'function') {
        updateGIHPrintButtonState();
    }
    if (typeof updateGIHRoomsSummary === 'function') {
        updateGIHRoomsSummary();
    }
});
                    updateGIHSummary();;
    ...
    try {
        ...
        if (firebaseData) {
            ...
            // Firebase пуст - инициализируем пустые данные
            // НЕ загружаем из localStorage - данные всегда должны быть в БД
            console.log('Firebase пуст, инициализируем пустые данные');
        }
    } catch (error) {
        // Ошибка Firebase - инициализируем пустые данные
        // НЕ загружаем из localStorage - данные всегда должны быть в БД
        console.error('Ошибка загрузки из Firebase, инициализируем пустые данные:', error);
    }
    ...
    setupDailyHistoryListener();
    setupDeadlinesStatsRealtimeListener(); // Включаем realtime синхронизацию статистики

    // Загружаем статистику и проверяем цель на сегодня
    await loadDeadlinesStatsFromFirebase();
    await ensureDailyGoalCalculated();
...
// ОТКЛЮЧЕНО: данные всегда должны быть в БД, не сохраняем в localStorage
function backupHistory() {
    // НЕ сохраняем историю в localStorage - данные всегда должны быть в БД
    // if (appData.gihHistory && appData.gihHistory.length > 0) {
...
// ОТКЛЮЧЕНО: данные всегда должны быть в БД, не загружаем из localStorage
function restoreHistoryIfNeeded() {
    // НЕ восстанавливаем историю из localStorage - данные всегда должны быть в БД
...
<script>
                ...
                // Загружаем сохранённый подраздел из localStorage
                let historySubtab = localStorage.getItem('historySubtab') || 'gih'; // 'gih' или 'daily'
                let historySortMode = localStorage.getItem('historySortMode') || 'rooms'; // 'rooms' или 'time'
                let dailyHistoryData = {}; // Данные ежедневных проверок: { 'YYYY-MM-DD': { room: { ts } } }
                let dailyHistoryRef = null;

                // Функция для форматирования времени из timestamp (UTC+5, Екатеринбург)
                function timeStringFromUnix(tsSec) {
                    if (!tsSec || typeof tsSec !== 'number') return '';
                ...
                // Функция для форматирования даты в ключ формата YYYY-MM-DD
    ...
    const gihBtn = document.getElementById('history-subtab-gih');
    const dailyBtn = document.getElementById('history-subtab-daily');
    
    if (!prevBtn || !nextBtn || !dateDisplay) return;

    // Обработчики для кнопок переключения подразделов
    if (gihBtn) {
        gihBtn.addEventListener('click', () => switchHistorySubtab('gih'));
    }
    if (dailyBtn) {
        dailyBtn.addEventListener('click', () => switchHistorySubtab('daily'));
    }

    // Обработчик для кнопки сортировки
    const sortToggleBtn = document.getElementById('history-sort-toggle');
    if (sortToggleBtn) {
        sortToggleBtn.addEventListener('click', toggleHistorySortMode);
    }

    // Инициализируем активную кнопку (используем сохранённое значение или 'gih' по умолчанию)
    switchHistorySubtab(historySubtab);

    // Инициализируем режим сортировки
    ...
    function updateDateDisplay() {
        ...
        // Добавляем количество записей/номеров
        if (historySubtab === 'daily') {
            // Для подраздела "Ежедневные" добавляем количество проверенных номеров
            const dateKey = dateKeyFromDate(historyCurrentDate);
            const dayData = dailyHistoryData[dateKey] || {};
            const count = Object.keys(dayData).length;
            if (count > 0) {
                dateText += ` (${count})`;
            }
        } else if (historySubtab === 'gih') {
            // Для подраздела "GIH" добавляем количество записей за выбранную дату
    ...
    updateDateDisplay();
                ...
                function renderHistory() {
                    // Если выбран подраздел ежедневных проверок, используем другую функцию
                    if (historySubtab === 'daily') {
                        renderDailyHistory();
                        return;
                    }
                    ...
                    historyList.innerHTML = '';
                    // Убираем класс floor-mode для подраздела GIH
                    historyList.classList.remove('floor-mode');
                    ...
                    // Сортируем в зависимости от выбранного режима
                    if (historySortMode === 'time') {
                        // Сортировка по времени (новые сверху)
                        filteredHistory.sort((a, b) => new Date(b.savedAt || b.createdAt) - new Date(a.savedAt || a.createdAt));
                    } else {
                        // Сортировка по номерам (режим 'rooms' - по умолчанию)
                ...
                // Функция для группировки номеров по этажам (для ежедневных проверок)
                function groupRoomsByFloorForDaily(rooms) {
                    const floors = {};
                    rooms.forEach(r => {
                ...
                // Функция для отображения ежедневных проверок
                function renderDailyHistory() {
                    const historyList = document.getElementById('history-list');
                    if (!historyList) return;

                    historyList.innerHTML = '';
                    // Добавляем класс floor-mode для отображения этажей в отдельных строках
                    historyList.classList.add('floor-mode');

                    const selectedDateKey = dateKeyFromDate(historyCurrentDate);
                    const dayData = dailyHistoryData[selectedDateKey] || {};

                    const rooms = Object.entries(dayData).map(([room, entry]) => ({
                        room: room,
                        time: timeStringFromUnix(entry.ts || 0),
                        ts: entry.ts || 0
                    }));
                    ...
                    if (historySortMode === 'time') {
                        ...
                        rooms.sort((a, b) => a.ts - b.ts);
                    } else {
                        // Сортировка по номерам (режим 'rooms' - по умолчанию)
                        rooms.sort((a, b) => parseInt(a.room) - parseInt(b.room));
                    }

                    if (rooms.length === 0) {
                        historyList.innerHTML = '<div class="muted" style="padding:20px; text-align:center;">За выбранный день записей нет</div>';
                        historyList.classList.remove('floor-mode');
                        historyList.classList.remove('time-mode');
                        return;
                    }

                    // В режиме "По времени" не группируем по этажам, показываем плоский список
                    if (historySortMode === 'time') {
                        historyList.classList.remove('floor-mode');
                        historyList.classList.add('time-mode');
                        ...
                        rooms.forEach(r => {
                            ...
                            // Проверяем, является ли номер опустошённым
                            const roomData = (appData.rooms || []).find(room => room && (room.number == r.room || room.name == r.room));
                            const isEmptied = roomData && roomData.deadlinesStatus === DEADLINE_STATUS.AUTO_OK;

                            // Добавляем класс для голубого фона, если номер опустошён
                            if (isEmptied) {
                                card.classList.add('emptied');

                                // Значок опустошения (∅) в левом нижнем углу
                        ...
                        // Группируем по этажам в режиме "По номерам"
                        historyList.classList.add('floor-mode');
                        historyList.classList.remove('time-mode');
                        const floors = groupRoomsByFloorForDaily(rooms);
                    ...
                    sortedFloors.forEach(floor => {
                        ...
                        // Сортируем комнаты в этаже в зависимости от режима
                        ...
                        historyList.appendChild(floorSection);
                    });
                    } // Закрываем блок else для режима "По номерам"
                }

                // Функция для настройки Firebase listener для ежедневных проверок
                function setupDailyHistoryListener() {
                    if (!database || typeof database.ref !== 'function') return;
                    if (dailyHistoryRef) return;
                    
                    dailyHistoryRef = database.ref('vkCheckedRoomsByDate');
                    dailyHistoryRef.on('value', snapshot => {
                        const data = snapshot.val() || {};
                        dailyHistoryData = data;
                        // Делаем доступным глобально для функции печати
                        window.dailyHistoryData = data;
                        if (historySubtab === 'daily') {
                            renderDailyHistory();
                            updateDateDisplay();
                        }
                        // Обновляем календарь при изменении данных (если календарь уже инициализирован)
                        setTimeout(() => {
                            if (typeof customCalendar !== 'undefined' && customCalendar) {
                ...
                // Функция для переключения подразделов истории
                function switchHistorySubtab(subtab) {
                    historySubtab = subtab;
                    
                    // Сохраняем выбранный подраздел в localStorage
                    localStorage.setItem('historySubtab', subtab);
                    ...
                    if (gihBtn && dailyBtn) {
                        if (subtab === 'gih') {
                            gihBtn.classList.remove('ghost');
                            dailyBtn.classList.add('ghost');
                        } else {
                            dailyBtn.classList.remove('ghost');
                            gihBtn.classList.add('ghost');
                        }
                    }

                    // Проверяем, активен ли поиск (есть ли значение в поле поиска)
                    const searchInput = document.getElementById('history-search-input');
                    const searchValue = searchInput ? searchInput.value.trim() : '';
                    
                    if (searchValue) {
                        // Если поиск активен, перезапускаем поиск для нового подраздела
                        // Используем событие input, чтобы триггерить поиск
                        if (searchInput) {
                            searchInput.dispatchEvent(new Event('input', { bubbles: true }));
                        }
                    } else {
                        // Если поиск не активен, просто перерисовываем историю
                        if (subtab === 'gih') {
                            renderHistory();
                        } else {
                            renderDailyHistory();
                ...
                // Функция для переключения режима сортировки истории
                function toggleHistorySortMode() {
                    historySortMode = historySortMode === 'time' ? 'rooms' : 'time';
                    localStorage.setItem('historySortMode', historySortMode);
                    updateHistorySortButton();

                    // Перерисовываем историю с новым режимом сортировки
                    if (historySubtab === 'gih') {
                        renderHistory();
                    } else {
                        renderDailyHistory();
                    }
                ...
                // Функция для обновления текста кнопки сортировки
                function updateHistorySortButton() {
                    const sortLabel = document.getElementById('history-sort-label');
                    if (sortLabel) {
                        sortLabel.textContent = historySortMode === 'time' ? 'По времени' : 'По номерам';
                    }
                }
...
<script>
                ...
                (function () {
                    ...
                    function enterSearchMode() {
                        ...
                        // Скрываем кнопку сортировки при входе в режим поиска
                        const sortToggleBtn = document.getElementById('history-sort-toggle');
                        if (sortToggleBtn) {
                    ...
                    function exitSearchMode() {
                        ...
                        // Убираем floor-mode если был добавлен при поиске
                        if (typeof historySubtab !== 'undefined' && historySubtab === 'daily') {
                            historyList.classList.add("floor-mode");
                        } else {
                            historyList.classList.remove("floor-mode");
                        }
                        
                        // Показываем кнопку сортировки обратно при выходе из режима поиска
                        const sortToggleBtn = document.getElementById('history-sort-toggle');
                        if (sortToggleBtn) {
                    ...
                    function renderSearchResults(query) {
                        ...
                        // Убеждаемся, что кнопка сортировки скрыта при активном поиске
                        const sortToggleBtn = document.getElementById('history-sort-toggle');
                        if (sortToggleBtn) {
                            sortToggleBtn.style.display = 'none';
                        }

                        // Если выбран подраздел "Ежедневные", ищем по ежедневным проверкам
                        if (typeof historySubtab !== 'undefined' && historySubtab === 'daily') {
                            renderDailySearchResults(cleanQuery);
                            return;
                        }

                        const regex = new RegExp('\\b' + escapeRegExp(cleanQuery) + '\\b', 'i');

                        /* фильтруем по всей истории GIH */
                    ...
                    // Функция для поиска по ежедневным проверкам
                    function renderDailySearchResults(query) {
                        const historyList = document.getElementById("history-list");
                        const cleanQuery = query.trim();
                        ...
                        // Функция группировки по этажам (локальная копия для поиска)
                        ...
                        if (!dailyHistoryData || typeof dailyHistoryData !== 'object') {
                            list.innerHTML = '<div class="muted" style="padding:20px; text-align:center;">Ничего не найдено</div>';
                            return;
                        }

                        // Пытаемся распарсить запрос как дату (различные форматы)
                        let searchDate = null;
                        const dateFormats = [
                        ...
                        for (const format of dateFormats) {
                            const match = cleanQuery.match(format);
                            if (match) {
                                if (format === dateFormats[0]) { // DD.MM.YYYY
                                    searchDate = new Date(parseInt(match[3]), parseInt(match[2]) - 1, parseInt(match[1]));
                                } else if (format === dateFormats[1]) { // DD.MM
                                    const today = new Date();
                                    searchDate = new Date(today.getFullYear(), parseInt(match[2]) - 1, parseInt(match[1]));
                                } else if (format === dateFormats[2]) { // YYYY-MM-DD
                                    searchDate = new Date(parseInt(match[1]), parseInt(match[2]) - 1, parseInt(match[3]));
                                }
                                break;
                            }
                        }
                        ...
                        // Проходим по всем датам в dailyHistoryData
                        Object.keys(dailyHistoryData).forEach(dateKey => {
                            const dayData = dailyHistoryData[dateKey] || {};
                            const dateObj = new Date(dateKey + 'T00:00:00');
                            const dateStr = dateObj.toLocaleDateString('ru-RU');
                            
                            // Проверяем, соответствует ли дата запросу
                            let dateMatches = false;
                            if (searchDate) {
                                const searchDateStr = searchDate.toLocaleDateString('ru-RU');
                                dateMatches = dateStr === searchDateStr || dateKey.includes(cleanQuery);
                            } else {
                                dateMatches = dateStr.includes(cleanQuery) || dateKey.includes(cleanQuery);
                            }

                            // Проходим по всем номерам за эту дату
                            Object.keys(dayData).forEach(room => {
                                const entry = dayData[room];
                                const roomMatches = regex.test(room);
                                
                                if (roomMatches || dateMatches) {
                                    if (!matchedDates[dateKey]) {
                                        matchedDates[dateKey] = [];
                                    }
                                    matchedDates[dateKey].push({
                                        room: room,
                                        time: typeof timeStringFromUnix === 'function' ? timeStringFromUnix(entry.ts || 0) : '',
                                        ts: entry.ts || 0
                                    });
                                }
                            });
                        ...
                        const sortedDates = Object.keys(matchedDates).sort((a, b) => {
                        ...
                        sortedDates.forEach(dateKey => {
                            ...
                            const rooms = matchedDates[dateKey];
...
class CustomCalendar {
    ...
    getRecordsCountForDate(date) {
        // Если выбран подраздел "Ежедневные", считаем количество проверенных номеров
        if (typeof historySubtab !== 'undefined' && historySubtab === 'daily') {
            if (!dailyHistoryData || typeof dailyHistoryData !== 'object') return 0;
            
            // Форматируем дату в ключ формата YYYY-MM-DD
            const y = date.getFullYear();
            const m = String(date.getMonth() + 1).padStart(2, '0');
            const d = String(date.getDate()).padStart(2, '0');
            const dateKey = `${y}-${m}-${d}`;
        ...
        // Для подраздела GIH считаем записи из gihHistory
...
// Обновляем функцию отображения даты - с учетом подраздела
function updateDateDisplay() {
    const dateDisplay = document.getElementById('history-current-date');
    if (!dateDisplay) return;
    ...
    // Добавляем количество записей/номеров
    if (typeof historySubtab !== 'undefined') {
        if (historySubtab === 'daily') {
            // Для подраздела "Ежедневные" добавляем количество проверенных номеров
            if (typeof dateKeyFromDate === 'function' && typeof dailyHistoryData !== 'undefined') {
                const dateKey = dateKeyFromDate(historyCurrentDate);
                const dayData = dailyHistoryData[dateKey] || {};
                const count = Object.keys(dayData).length;
                if (count > 0) {
            ...
            // Для подраздела "GIH" добавляем количество записей за выбранную дату
            if (typeof appData !== 'undefined' && appData.gihHistory) {
                const selectedDateStr = historyCurrentDate.toLocaleDateString('ru-RU');
                const filteredHistory = (appData.gihHistory || []).filter(entry => {
                    if (!entry.createdAt) return false;
                    const entryDate = new Date(entry.createdAt).toLocaleDateString('ru-RU');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function printHistory() {
    ...
    // Определяем, какой подраздел активен (GIH или ежедневные)
    // Получаем из DOM или localStorage
    const historySubtabGihBtn = document.getElementById('history-subtab-gih');
    const historySubtabDailyBtn = document.getElementById('history-subtab-daily');
    const isDaily = historySubtabDailyBtn && !historySubtabDailyBtn.classList.contains('ghost');
    
    if (isDaily) {
      // Для ежедневных проверок
      // Получаем текущую дату из DOM
      const currentDateEl = document.getElementById('history-current-date');
      let selectedDate = new Date();
      if (currentDateEl && currentDateEl.dataset.date) {
        selectedDate = new Date(currentDateEl.dataset.date);
      }
      
      const y = selectedDate.getFullYear();
      const m = String(selectedDate.getMonth() + 1).padStart(2, '0');
      const d = String(selectedDate.getDate()).padStart(2, '0');
      const selectedDateKey = `${y}-${m}-${d}`;
      
      // Получаем данные ежедневных проверок из глобальной переменной или из localStorage
      // Попытаемся получить из window, если не определено - используем пустой объект
      let dailyHistoryData = {};
      try {
        // Пытаемся получить из контекста выполнения (если переменная доступна)
        if (typeof dailyHistoryData !== 'undefined' && typeof dailyHistoryData === 'object') {
          // Используем существующую переменную
        } else if (typeof window !== 'undefined' && window.dailyHistoryData) {
          dailyHistoryData = window.dailyHistoryData;
        }
      } catch (e) {
        console.warn('Не удалось получить dailyHistoryData:', e);
      }
      
      const dayDataForDate = dailyHistoryData[selectedDateKey] || {};
      
      // Форматируем данные
      historyData = Object.entries(dayDataForDate).map(([room, entry]) => {
        // Форматируем время из timestamp
        let time = '';
        if (entry && entry.ts) {
          const tsSec = entry.ts;
        ...
        // Ищем соответствующую запись в gihHistory для получения продуктов/режима
        let productsInfo = '';
        const gihHistory = (window.appData && window.appData.gihHistory) || [];
        if (gihHistory.length > 0) {
          const gihEntry = gihHistory.find(e => {
            ...
            return entryDateKey === selectedDateKey && (String(e.room || e.number) === String(room));
          });
          
          if (gihEntry) {
            if (gihEntry.statusMode) {
              const modeLabels = {
                dnd: 'DND',
                allInPlace: 'Всё на месте',
                putOut: 'Всё выложили',
              ...
              productsInfo = modeLabels[gihEntry.statusMode] || gihEntry.statusMode;
            } else if (gihEntry.products && gihEntry.products.length > 0) {
              const byStatus = {};
              gihEntry.products.forEach(p => {
                const st = Number(p.status || 0);
                const name = (window.appData && window.appData.products && window.appData.products[p.name]) || p.name;
                byStatus[st] = byStatus[st] || {};
                byStatus[st][name] = (byStatus[st][name] || 0) + 1;
              });
              ...
              [1, 2, 3, 4].forEach(status => {
                if (byStatus[status]) {
                  const products = Object.entries(byStatus[status])
                    ...
                    .join(', ');
                  if (products) {
                    parts.push(`${statusLabels[status]}: ${products}`);
                  }
                }
              });
              productsInfo = parts.join('; ');
            }
          }
        }
      ...
      historyData.sort((a, b) => parseInt(a.number) - parseInt(b.number));
    } else {
      // Для GIH истории
      // Получаем текущую дату из DOM или используем сегодняшнюю
      const currentDateEl = document.getElementById('history-current-date');
      let selectedDate = new Date();
      if (currentDateEl && currentDateEl.dataset.date) {
        selectedDate = new Date(currentDateEl.dataset.date);
      ...
      const gihHistory = (window.appData && window.appData.gihHistory) || [];
      const filteredHistory = gihHistory.filter(entry => {
        if (!entry.createdAt) return false;
        const entryDate = new Date(entry.createdAt).toLocaleDateString('ru-RU');
        return entryDate === selectedDateStr;
      });
      
      historyData = filteredHistory.map(entry => {
        const roomNumber = entry.room || entry.number || '';
        let savedAt = entry.savedAt || '';
        if (!savedAt && entry.createdAt) {
        ...
        if (entry.statusMode) {
          ...
          productsInfo = modeLabels[entry.statusMode] || entry.statusMode;
        } else if (entry.products && entry.products.length > 0) {
          const byStatus = {};
          entry.products.forEach(p => {
            const st = Number(p.status || 0);
            const name = (window.appData && window.appData.products && window.appData.products[p.name]) || p.name;
      ...
      // Сортируем в зависимости от режима сортировки (из localStorage или по умолчанию)
      const sortMode = localStorage.getItem('historySortMode') || 'rooms';
      if (sortMode === 'time') {
    ...
    // Разбиваем на страницы (72 записи на страницу, по 36 в каждой колонке)
    const itemsPerPage = 72;
    ...
    let htmlContent = `
        ...
        <html>
        <head>
            <title>History ${dateString}</title>
            <style>
                @page {
                    size: A4 landscape;
                    margin: 1cm;
                ...
                .print-table .guest-col {
                    width: 100px;
                    text-align: left;
                    padding-left: 4px;
                }
                .print-table .arrival-col {
                    width: 80px;
                    text-align: center;
                }
                .print-table .departure-col {
                    width: 80px;
                    text-align: center;
                }
    ...
    for (let pageNum = 0; pageNum < pageCount; pageNum++) {
      const pageStart = pageNum * itemsPerPage;
      const pageEnd = pageStart + itemsPerPage;
      const pageItems = historyData.slice(pageStart, pageEnd);
                ...
                <div class="page-column">
                    <table class="print-table">
                        <thead>
                            <tr>
                                <th class="number-col"><font size="2">№</font></th>
                                <th class="guest-col"><font size="2">Гость</font></th>
                                <th class="arrival-col"><font size="2">Дата заезда</font></th>
                                <th class="departure-col"><font size="2">Дата выезда</font></th>
                                <th class="time-col"><font size="2">Время</font></th>
                                <th class="products-col"><font size="2">Продукты/Режим</font></th>
                            </tr>
                        </thead>
                        <tbody>
      `;
      ...
      for (let i = 0; i < 36; i++) {
        ...
        if (item) {
          htmlContent += `
                            <tr>
                                <td class="number-col"><strong><font size="2">${escapeHtml(item.number)}</font></strong></td>
                                <td class="guest-col">&nbsp;</td>
                                <td class="arrival-col">&nbsp;</td>
                                <td class="departure-col">&nbsp;</td>
                                <td class="time-col"><font size="1">${escapeHtml(item.time || '')}</font></td>
                                <td class="products-col"><font size="1">${escapeHtml(item.products || '')}</font></td>
                            </tr>
          `;
        } else {
          htmlContent += `
  ...
  function printGIH() {
    ...
    // Получаем все сохраненные записи GIH
    const records = (window.appData && window.appData.gihRecords) || [];
    const savedRecords = records.filter(r => r && r.savedAt);
    
    if (savedRecords.length === 0) {
      alert('Нет сохраненных записей для печати');
      return;
    }
    
    // Форматируем данные
    const historyData = savedRecords.map(record => {
      ...
      if (record.statusMode) {
        ...
        const modeLabel = modeLabels[record.statusMode] || record.statusMode;
        
        // Если выбран режим, добавляем продукты в скобках
        if (record.products && record.products.length > 0) {
          ...
          const productList = Object.entries(productGroups)
            .map(([name, count]) => count > 1 ? `${name} x${count}` : name)
            .join(', ');
          productsInfo = `${modeLabel} (${productList})`;
        } else {
          productsInfo = modeLabel;
        }
      } else if (record.products && record.products.length > 0) {
        const byStatus = {};
        record.products.forEach(p => {
          const st = Number(p.status || 0);
          const name = (window.appData && window.appData.products && window.appData.products[p.name]) || p.name;
    ...
    // Разбиваем на страницы (50 записей на страницу, по 25 в каждой колонке)
    ...
    for (let pageNum = 0; pageNum < pageCount; pageNum++) {
      ...
      const left = pageItems.slice(0, 25);
      const right = pageItems.slice(25, 50);
      
      while (left.length < 25) left.push(null);
      while (right.length < 25) right.push(null);
                ...
                <div class="page-column">
                    <table class="print-table">
                        <thead>
                            <tr>
                                <th class="number-col"><font size="2">№</font></th>
                                <th class="time-col"><font size="2">Time</font></th>
                                <th class="products-col" style="text-align: center;"><font size="2">GIH of ${dateString}</font></th>
                            </tr>
                        </thead>
                        <tbody>
      `;
      ...
      for (let i = 0; i < 25; i++) {
        const item = left[i];
        if (item) {
          // Определяем, является ли содержимое режимом (начинается с определенных слов)
          const isMode = item.products && (
            item.products.startsWith('DND') ||
            item.products.startsWith('Всё на месте') ||
            item.products.startsWith('Всё выложили') ||
            item.products.startsWith('Опустошён')
          );
          
          let productsHtml = '';
          if (isMode) {
            // Если это режим, выделяем его жирным
            const modeMatch = item.products.match(/^([^(]+)(\(.+\))?/);
            if (modeMatch) {
              const modeText = modeMatch[1].trim();
              const productsText = modeMatch[2] || '';
              productsHtml = `<strong>${escapeHtml(modeText)}</strong> ${escapeHtml(productsText)}`;
            } else {
              productsHtml = `<strong>${escapeHtml(item.products)}</strong>`;
            }
          } else if (item.products) {
            // Если это статусы продуктов, выделяем названия статусов жирным
            // Формат: "Пополнено: Твикс x2, Кола; На месте: Ред Булл"
            // Сначала экранируем весь текст
            let escaped = escapeHtml(item.products);
            // Затем заменяем экранированные названия статусов на жирные версии
            escaped = escaped.replace(
              /(Пополнено|На месте|Выложили|Не пополнено)(\s*:\s*)/g,
              '<strong>$1</strong>$2'
            );
            productsHtml = escaped;
          } else {
            productsHtml = escapeHtml(item.products || '');
          }
          ...
          htmlContent += `
                            <tr>
                                <td class="number-col"><strong><font size="2">${escapeHtml(item.number)}</font></strong></td>
                                <td class="time-col"><font size="1">${escapeHtml(item.time || '')}</font></td>
                                <td class="products-col"><font size="1">${productsHtml}</font></td>
                            </tr>
          `;
        } else {
          htmlContent += `
  ...
  // Функция для обновления состояния кнопки печати
  function updateGIHPrintButtonState() {
    const printGihBtn = document.getElementById('print-gih-btn');
    if (!printGihBtn) return;
    
    const records = (window.appData && window.appData.gihRecords) || [];
    const total = records.length;
    const saved = records.filter(r => r && r.savedAt).length;
    
    // Кнопка активна только если есть записи и все они сохранены
    printGihBtn.disabled = total === 0 || saved !== total;
  }
  
  // Делаем функцию доступной глобально
  window.updateGIHPrintButtonState = updateGIHPrintButtonState;
  
  const printGihBtn = document.getElementById('print-gih-btn');
  if (printGihBtn && window.innerWidth >= 921) {
    printGihBtn.addEventListener('click', printGIH);
...
(function() {
  ...
  <span>Выписать сроки</span>
    ...
    if (room.deadlinesStatus === DEADLINE_STATUS.OK || room.deadlinesStatus === DEADLINE_STATUS.AUTO_OK) return ''; // сроки в порядке — ничего не дописываем
...
function aggregateDeadlineProductsForRoomNumbers(roomNumbers) {
    ...
    (roomNumbers || []).forEach(rn => {
        ...
        if (!room || room.deadlinesStatus === DEADLINE_STATUS.OK || room.deadlinesStatus === DEADLINE_STATUS.AUTO_OK) return; // пропускаем OK
...
function printDepartures() {
    ...
    function buildDeadlineSubForRoom(roomNumber) {
        ...
        if (!room) {
            console.log(`Room ${roomNumber}: no data in appData.rooms`);
            return 'check';
        }

        const status = room.deadlinesStatus;
        console.log(`Room ${roomNumber}: status = ${status}, includeDeadlines = ${includeDeadlines}`);

        if (status === DEADLINE_STATUS.PRODUCTS) {
            console.log(`Room ${roomNumber}: PRODUCTS branch`);
            // Печатаем продукты, если выбран переключатель "Выписать сроки"
            if (includeDeadlines) {
                ...
                const result = entries
                    ...
                    .join(', ');
                console.log(`Room ${roomNumber}: returning products: ${result}`);
                return result;
            } else {
                // Если переключатель не выбран, ничего не печатаем
                console.log(`Room ${roomNumber}: PRODUCTS but includeDeadlines=false, returning empty`);
                return '';
            }
        } else if (status === DEADLINE_STATUS.AUTO_OK) {
            console.log(`Room ${roomNumber}: AUTO_OK branch, returning 'пустой'`);
            // Всегда печатаем "пустой" независимо от переключателя
            return 'пустой';
        } else if (status === DEADLINE_STATUS.OK) {
            console.log(`Room ${roomNumber}: OK branch, returning empty`);
            // Для ручного OK ничего не печатаем
            return '';
        } else if (status === DEADLINE_STATUS.NEUTRAL) {
            console.log(`Room ${roomNumber}: NEUTRAL branch`);
            // Печатаем "сроки", если выбран переключатель "Выписать сроки"
            const result = includeDeadlines ? 'сроки' : '';
            console.log(`Room ${roomNumber}: returning ${result ? 'сроки' : 'empty'}`);
            return result;
        }

        // По умолчанию — проверка
        console.log(`Room ${roomNumber}: default branch, returning 'check'`);
        return 'check';
    }
    ...
    pages.forEach((page, pageIndex) => {
        ...
        for (let i = 0; i < 36; i++) {
            ...
            if (item) {
                ...
                // Проверяем, был ли номер помечен как опустошённый
                let isEmptied = false;

                // Сначала проверяем статус в appData.rooms (это самый надежный способ)
                const roomData = (appData.rooms || []).find(r => r && String(r.number) === String(item.number));
                if (roomData && roomData.deadlinesStatus === DEADLINE_STATUS.AUTO_OK) {
                    isEmptied = true;
                }

                // Дополнительно проверяем todayRoomsData
                if (!isEmptied && window.todayRoomsData && Array.isArray(window.todayRoomsData)) {
